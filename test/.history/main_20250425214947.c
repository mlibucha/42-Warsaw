...
{
    int a_pos = find_insert_position(a, value);
    int cost_a = (a_pos <= a->size / 2) ? a_pos : a->size - a_pos;
    int cost_b = (b_pos <= b_size / 2) ? b_pos : b_size - b_pos;
    return cost_a + cost_b;
}

void rotate_to_push(t_stack *a, t_stack *b, t_node *target)
{
    int a_pos = find_insert_position(a, target->value);
    int b_pos = find_node_position(b, target);
    
    while (a->top->value != target->value && b->top != target)
    {
        if (a_pos <= a->size / 2 && b_pos <= b->size / 2)
            ...
            break;
        a_pos = find_insert_position(a, target->value);
        b_pos = find_node_position(b, target);
    }
    
    while (a->top->value != target->value)
    {
        if (a_pos <= a->size / 2)
            ra(a);
        else
...
{
    int min_pos = find_min_position(a);
    while (min_pos > 0 && min_pos <= a->size / 2)
    {
        ra(a);
        min_pos--;
    }
    while (min_pos > a->size / 2 && min_pos < a->size)
    {
        rra(a);
        min_pos++;
    }
...
void sort_large(t_stack *a, t_stack *b)
{
    ...
    while (b->size > 0)
    {
        ...
        t_node *best_node = b->top;
        int current_pos = 0;
        
        while (current)
        {
            int score = calculate_insert_score(a, current->value, current_pos, b->size);
            if (score < best_score)
            {
                best_score = score;
                best_node = current;
...
{
    t_node *temp;
    int i = count - 1; // Fill in reverse order
...
static int compare_values(const void *a, const void *b) {
    return ((t_indexed *)a)->value - ((t_indexed *)b)->value;
}

void index_tab(int *tablica, int count) {
    t_indexed *temp = malloc(sizeof(t_indexed) * count);
    if (!temp) return;
    
    // Create temporary array with original values and indices
    for (int i = 0; i < count; i++) {
        temp[i].value = tablica[i];
        temp[i].original_index = i;
    }
    
    // Sort the temporary array
    qsort(temp, count, sizeof(t_indexed), compare_values);
    
    // Create the indexed version
    for (int i = 0; i < count; i++) {
        tablica[temp[i].original_index] = i;
    }
