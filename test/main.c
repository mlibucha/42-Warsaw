...
int	is_sorted(t_stack *stack)// to samo
{
	t_node	*current;
...
int find_position(t_stack *a, int value)// zadnych z
{
	t_node	*current;
	int	i;
...
int	main(int argc, char **argv)//zmiana tylko z parsowanie i ten if sorted 
{
	t_stacks	*stack;
	int			*tablica;
	int			count;
...
/*
większość kodu to samo co wcześniej

zmiany w parsing z powodu tego dodatkowego staka na staki 

z 2? nowe funkcje radix_sort zamiast sort big i free_and_exit
